...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    //var topTenSampleValues = result.map( otu =>result.otu_ids).slice(0,10).reverse();

    var yticks = otu_ids.map(x => "OTU " + x.toString()).slice(0, 10).reverse();
    ...
    var gaugeData = [
      {
        ...
        gauge: {
          axis: { range: [0, 10] },
          bar: { color: "black" },
          steps: [{ range: [0, 2], color: "lightsteelblue" },
